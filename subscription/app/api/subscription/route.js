...
const allowed_status = ["active", "inactive", "cancelled"];
...
export async function POST(req) {
	...
	if (!allowed_frequencies.includes(frequency)) {
		return new Response(
			JSON.stringify({ error: "Frequency is not permitted." }),
			{ status: 400 }
		);
	}
	...
	if (typeof businessDaysOnly !== "boolean") {
		return new Response(
			JSON.stringify({ error: "Business days only must be a boolean" }),
			{ status: 400 }
		);
	}
	if (notes && notes.length > 500) {
		return new Response(
			JSON.stringify({ error: "Notes cannot exceed 500 characters" }),
			{ status: 400 }
		);
	}
	if (
		...
		return new Response(
			JSON.stringify({ error: "Notes contain invalid characters" }),
			{ status: 400 }
		);
	}
	if (!allowed_categories.includes(category)) {
		return new Response(
			JSON.stringify({ error: "Category is not permitted." }),
			{ status: 400 }
		);
	}
	...
	if (paymentMethod.length > 20 || paymentMethod.length < 3) {
		return new Response(
			JSON.stringify({
				error: "Payment method must be between 3 and 20 characters",
			}),
			{ status: 400 }
		);
	}
	if (!allowed_status.includes(status)) {
		return new Response(
			JSON.stringify({ error: "Status is not permitted." }),
			{ status: 400 }
		);
	}
	if (typeof autoRenew !== "boolean") {
		return new Response(
			JSON.stringify({ error: "Auto renew must be a boolean" }),
			{ status: 400 }
		);
	}
