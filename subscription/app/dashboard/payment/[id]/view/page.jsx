...
function processSubscription(subscription) {
	...
	const start_date = moment(subscription.start_date).utc();
	...
	const final_date = subscription.final_date
		? moment(subscription.final_date).utc()
		: null;
	...
	if (
		autorenew &&
		start_date.isValid() &&
		((final_date && final_date.isAfter(moment())) || !final_date) &&
		status === "active"
	) {
			...
			<div className="mapbg max-w-[30cm] mx-auto w-full md:w-1/2 min-w-fit py-4 px-4 rounded-2xl border-2 border-accent-foreground/40 bg-accent/30 shadow-xl my-2">
				...
				<div className="flex flex-col md:flex-row items-center justify-around my-4 max-w-[100%] gap-2">
					...
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						...
						<p>
							{getSymbol()}
							{processedSubscription.amount.toFixed(2)}
						</p>
					</div>
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						<p className="text-lg text-muted-foreground font-semibold">
						...
						<p>
							{moment(processedSubscription.start_date).format(
								"MMMM Do YYYY"
							)}
						</p>
					</div>
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						...
						<p className="first-letter:uppercase">
							{processedSubscription.frequency}
						</p>
					</div>
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						<p className="text-lg text-muted-foreground font-semibold">
						...
						<p className="first-letter:uppercase">
							{processedSubscription.autorenew
								? "Enabled"
								: "Disabled"}
						</p>
					</div>
				...
				<div className="flex flex-col md:flex-row items-center justify-around my-4 max-w-[100%] gap-2">
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						<p className="text-lg text-muted-foreground font-semibold text-center md:text-left">
							Business days only
						</p>
						<p className="first-letter:uppercase">
							{processedSubscription.business_days_only
								? "Yes"
								: "No"}
						</p>
					</div>
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						...
						<p className="first-letter:uppercase">
							{processedSubscription.category}
						</p>
					</div>
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						<p className="text-lg text-muted-foreground font-semibold">
					...
					<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
						...
						<p className="first-letter:uppercase">
							{processedSubscription.payment_method}
						</p>
					</div>
					{processSubscription.final_date && (
						<div className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto">
							...
							<p className="first-letter:uppercase">
								{processedSubscription.final_date || "N/A"}
							</p>
						</div>
					)}
				</div>
				...
				{processedSubscription.nextPayments.length > 0 && (
					<>
						...
						<div className="flex flex-wrap justify-center gap-4 mt-2">
							{processedSubscription.nextPayments.map(
								(date, index) => (
									<div
										key={index}
										className="flex flex-col items-center bg-accent/90 px-6 py-2 rounded-lg shadow-md border-2 w-[50%] md:w-auto"
									>
										<p>
											{moment(date).format(
												"MMMM Do YYYY"
