...
export default function InputForm() {
    ...
    const [date, setDate] = useState(moment().add(1, "day").toDate());
    ...
    function trySubmit() {
        ...
        const payload = {
            ...
            status,
            autoRenew,
            finalDate: finalDate ? moment(finalDate).format() : null
        };
    ...
    if (submitted) {
        return (
            <div className="text-center">
                ...
                <h2 className="text-2xl font-semibold">Subscription Added Successfully!</h2>
                <p className="mt-2">Thank you for adding your subscription.</p>
                <Button className="mt-4" onClick={() => setSubmitted(false)}>Add Another Subscription</Button>
            </div>
        );
    }
    ...
    return (
        <>
            ...
            <div className="grid w-full max-w-sm items-center gap-1">
                <Label htmlFor="date">Next Billing Date<Required /></Label>
                <Popover>
            ...
            <div className="grid w-full max-w-sm items-center gap-1">
                <Label htmlFor="frequency">Frequency<Required /></Label>
                ...
                <p className="text-muted-foreground text-sm">Next billing date: {moment(nextBillingDate).format("MMM D, YYYY")}</p>
            </div>
            <div className="flex items-center space-x-2">
                <Switch id="business-days-only" checked={businessDaysOnly} onCheckedChange={setBusinessDaysOnly} />
            ...
            <div className="grid w-full max-w-sm items-center gap-1">
                <Label htmlFor="payment-method">Payment Method<Required /></Label>
                <Input type="text" id="payment-method" placeholder="Payment Method" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} />
            </div>
            <div className="grid w-full max-w-sm items-center gap-1">
            ...
            <div className="grid w-full max-w-sm items-center gap-1">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        ...
                        >
                            <CalendarIcon />
                            {finalDate ? moment(finalDate).format("MMM D, YYYY") : <span>Last payment date</span>}
                        </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0">
                        <Calendar mode="single" selected={finalDate} onSelect={setFinalDate} disabled={date => date <= new Date()} />
                    </PopoverContent>
                </Popover>
            </div>
            <Button className="mt-4 cursor-pointer" onClick={trySubmit}>Submit</Button>
        </>
    )
}